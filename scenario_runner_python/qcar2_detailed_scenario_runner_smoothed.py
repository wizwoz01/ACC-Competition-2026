...
class LaneController:
    ...
    def __init__(self):
        ...
        # --- Control gains ---
        self.k_lat = 0.85  # Reduced from 1.00 for smoother steering
        self.k_head = 0.20  # Reduced from 0.25 for stability
        self.kd = 0.06  # Increased from 0.04 for damping
        self.max_steer = 0.60
...
def run_scenario(
    ...
    # PHASE 2 TUNING: Reduced to 0.4 m/s for oscillation damping
    # Lower speed allows smoother steering control and reduces inertia-induced oscillations
    ...
    def pure_pursuit_lookahead_m(segment_name: str) -> float:
        # PHASE 2 TUNING: Increased lookahead for smoother, less oscillatory steering
        if segment_name == "PARK":
            return 0.20 if compact_track else 0.30  # Increased from 0.18/0.28
        if segment_name == "TO_HUB":
            # Increased lookahead for stability during hub return
            return 0.20 if compact_track else 0.40  # Increased from 0.18/0.35
        # Default: increased lookahead for straighter path tracking
        return 0.35 if compact_track else 0.55  # Increased from 0.25/0.50
    ...
    last_set_wp_index = 0
    segment_just_switched = True  # Flag to allow initial reset to waypoint 0 on segment change
    frames_since_segment_switch = 0  # Counter to prevent seg_max_idx pollution on entry
    def set_waypoint_index_safe(idx):
        nonlocal last_set_wp_index, segment_just_switched, frames_since_segment_switch
        ...
        # EXCEPT when just switching segments - allow reset to 0
        # Also skip safety checks for first 1 frame after switch to prevent seg_max_idx corruption
        if active_name == "TO_HUB" and not segment_just_switched and frames_since_segment_switch > 0:
            try:
        ...
        try:
            ...
            if segment_just_switched:
                segment_just_switched = False  # After first set, enable safety checks
                frames_since_segment_switch = 0  # Reset counter
    ...
    # ------------------------------------------------------------------
    FOLLOW_WAYPOINTS_ONLY = False  # PHASE 3: Enable lane-following on all routes for better centering
    ...
    # ==================================================================
    # OSCILLATION INSTRUMENTATION - PHASE 1 BASELINE
    # ==================================================================
    from collections import deque
    
    # Initialize oscillation tracking
    CTE_VARIANCE_WINDOW = 40  # 40-frame rolling window for variance calculation
    STEER_HISTORY_WINDOW = 30  # for oscillation frequency analysis
    STRAIGHTAWAY_STEER_THRESHOLD = 0.20  # rad - steering angle threshold for straight detection
    CURVE_ENTRY_THRESHOLD = 0.35  # rad - entering curve zone
    
    cte_history = deque(maxlen=CTE_VARIANCE_WINDOW)  # Rolling CTE buffer
    steer_history = deque(maxlen=STEER_HISTORY_WINDOW)  # Rolling steering buffer
    mode_state_history = []  # Track mode transitions
    
    # Open oscillation metrics log file
    osc_log = open(os.path.join(log_dir, "oscillation_baseline.csv"), "w", newline="")
    osc_writer = csv.writer(osc_log)
    osc_writer.writerow([
        "time_s", "cte_m", "cte_variance_m2", "steer_raw_rad", 
        "steer_delta_rad", "speed_mps", "mode_zone", "target_steer_rad",
        "path_curvature_rad_m", "lookahead_m", "segment_name"
    ])
    ...
    try:
        ...
        while True:
            ...
            if not route_done:
                ...
                # Only update seg_max_idx after first frame of segment switch (prevent corruption)
                if frames_since_segment_switch > 0:
                    seg_max_idx = max(seg_max_idx, i_near)
                frames_since_segment_switch += 1
                ...
                if (t >= segment_hold_until) and (seg_complete or pp_done):
                    ...
                    if segment_idx >= len(route_segments):
                        ...
                        last_path_cte_m = 0.0
                        last_set_wp_index = 0
                        segment_just_switched = True  # FIX: Re-enable segment switch flag for safe waypoint reset
                        ...
                        if active_name == "TO_HUB":
                            ...
                            pure_pursuit.maxSteeringAngle = 0.50  # More steering for tighter tracking
                            set_waypoint_index_safe(0)  # Reset to waypoint 0 on TO_HUB entry
                        else:
            ...
            if route_done and not parking_active:
                ...
                # ==================================================================
                # PHASE 3: ADAPTIVE LOOKAHEAD - Curvature-based adjustment
                # ==================================================================
                # Estimate local path curvature to adjust lookahead dynamically
                local_curvature = 0.0
                try:
                    current_wpi = int(getattr(pure_pursuit, "wpi", 0)) if hasattr(pure_pursuit, "wpi") else 0
                    local_curvature = estimate_local_curvature(active_wp, current_wpi, step=3)
                except Exception:
                    local_curvature = 0.0
                
                # Adaptive lookahead thresholds
                CURVE_CURVATURE_HIGH = 0.30  # rad/m - tight curves
                CURVE_CURVATURE_MED = 0.15   # rad/m - moderate curves
                STRAIGHT_LOOKAHEAD = 0.35 if compact_track else 0.55  # Longer for straights
                CURVE_LOOKAHEAD = 0.25 if compact_track else 0.35     # Shorter for curves
                
                # Determine adaptive lookahead based on curvature
                if local_curvature > CURVE_CURVATURE_HIGH:
                    # Tight curve: use shorter lookahead
                    adaptive_la = CURVE_LOOKAHEAD
                elif local_curvature > CURVE_CURVATURE_MED:
                    # Moderate curve: blend between straight and curve lookahead
                    blend_factor = (local_curvature - CURVE_CURVATURE_MED) / (CURVE_CURVATURE_HIGH - CURVE_CURVATURE_MED)
                    adaptive_la = blend_factor * CURVE_LOOKAHEAD + (1.0 - blend_factor) * STRAIGHT_LOOKAHEAD
                else:
                    # Straight: use longer lookahead
                    adaptive_la = STRAIGHT_LOOKAHEAD
                
                if active_name == "TO_HUB":
                    # Adaptive lookahead for return-to-hub: use curvature-based blending only
                    # Remove CTE override to prevent jerky lookahead changes causing oscillations
                    hub_la_min = 0.25 if compact_track else 0.35  # Minimum lookahead
                    hub_la_max = 0.40 if compact_track else 0.55  # Increased max for smoother path tracking
                    
                    # Use adaptive lookahead based purely on curvature
                    hub_la_des = adaptive_la
                    
                    hub_la_prev = float(getattr(pure_pursuit, "lookahead", base_lookahead_m))
                    # Apply smoothing to prevent jerky changes
                    pure_pursuit.lookahead = clamp(
                        LOOKAHEAD_SMOOTH_ALPHA * hub_la_prev + LOOKAHEAD_SMOOTH_BETA * hub_la_des,
                        hub_la_min,
                        hub_la_max,
                    )
                elif active_name == "TO_DROPOFF":
                    # TO_DROPOFF exits roundabout: use curvature-based lookahead only (no CTE override)
                    # CTE override was causing oscillations - stick with curvature blending
                    drop_la_min = 0.20 if compact_track else 0.28  # Increased min for stability
                    drop_la_max = 0.35 if compact_track else 0.45  # Increased max to prevent overshooting
                    
                    # Use adaptive lookahead based purely on curvature (ignore CTE)
                    drop_la_des = adaptive_la
                    
                    drop_la_prev = float(getattr(pure_pursuit, "lookahead", base_lookahead_m))
                    pure_pursuit.lookahead = clamp(
                        LOOKAHEAD_SMOOTH_ALPHA * drop_la_prev + LOOKAHEAD_SMOOTH_BETA * drop_la_des,
                        drop_la_min,
                        drop_la_max,
                    )
                    # Moderate speed reduction on TO_DROPOFF (previous 0.30 was too aggressive)
                    speed_cmd = min(speed_cmd, 0.32)  # Cap at 0.32 m/s for roundabout exit
                else:
                    # Default segments: use adaptive lookahead with CTE correction
                    if last_path_cte_m > MID_CTE_M:
                        la_des = max(0.25 if compact_track else 0.35, 0.85 * adaptive_la)
                    else:
                        la_des = adaptive_la
                    
                    la_prev = float(getattr(pure_pursuit, "lookahead", base_lookahead_m))
                    pure_pursuit.lookahead = clamp(
                        LOOKAHEAD_SMOOTH_ALPHA * la_prev + LOOKAHEAD_SMOOTH_BETA * la_des,
                        0.20 if compact_track else 0.30,
                        STRAIGHT_LOOKAHEAD,
                    )
                ...
                if active_name == "TO_HUB" and dist_to_goal < 4.0:
                    speed_cmd = min(speed_cmd, 0.35)  # Reduced from 0.38 for smoother entry
            ...
            if in_roundabout_zone_steer:
                ...
                # PHASE 3: Enable lane-following with adaptive blending on all routes
                # Pure pursuit provides path tracking, lane detection provides centering

                if drifted:
                    # When drifted, prioritize pure pursuit for recovery
                    steer_raw = clamp(target_steer, -0.48, 0.48)
                elif lane_ok and lane_conf >= 0.45 and not turning_active:
                    # Blend lane centering with pure pursuit for better stability
                    # Lane confidence 0.45+ provides reliable centering correction
                    centering_correction = clamp(-0.12 * lane_err, -0.08, 0.08)  # Increased blend range
                    steer_raw = clamp(target_steer + centering_correction, -0.50, 0.50)
                else:
                    # Default: pure path following when lane not available
            ...
            # ==================================================================
            # OSCILLATION TRACKING - Phase 1 Instrumentation
            # ==================================================================
            
            # Update CTE history for variance calculation
            cte_history.append(float(cte))
            
            # Calculate CTE variance (rolling window)
            if len(cte_history) >= 10:  # Need at least 10 samples
                cte_variance = float(np.var(cte_history))
            else:
                cte_variance = 0.0
            
            # Track steering oscillation
            steer_delta = 0.0
            if len(steer_history) > 0:
                steer_delta = float(abs(steer_raw - steer_history[-1]))
            steer_history.append(float(steer_raw))
            
            # Straightaway vs Curve Detection (using steering angle)
            abs_target_steer = abs(target_steer)
            
            if abs_target_steer < STRAIGHTAWAY_STEER_THRESHOLD:
                mode_zone = "STRAIGHT"
            elif STRAIGHTAWAY_STEER_THRESHOLD <= abs_target_steer < CURVE_ENTRY_THRESHOLD:
                mode_zone = "TRANS"  # Transition zone
            else:
                mode_zone = "CURVE"
            
            # Capture current lookahead and curvature for logging
            current_lookahead = float(getattr(pure_pursuit, "lookahead", 0.0)) if pure_pursuit is not None else 0.0
            
            # Log oscillation metrics with adaptive tuning info
            try:
                osc_writer.writerow([
                    t, 
                    cte, 
                    ...
                    mode_zone,
                    target_steer,
                    local_curvature if 'local_curvature' in locals() else 0.0,
                    current_lookahead,
                    active_name if 'active_name' in locals() else "UNKNOWN"
                ])
                osc_log.flush()  # Ensure data is written immediately
            except Exception as e:
                if debug_print and _dbg_loop_count % 200 == 0:
                    print(f"[WARN] Oscillation logging failed: {e}")

            if pure_pursuit is not None and hasattr(pure_pursuit, "p_ref"):
                lx, ly = pure_pursuit.p_ref[0], pure_pursuit.p_ref[1]
                lookahead_str = f"  lookahead=({lx:+.2f},{ly:+.2f})"
            # Debug HUD and telemetry display
            try:
                ...
                elif debug_print and _dbg_loop_count % 100 == 0:
                    print(f"[DEBUG] bgr is None or empty - camera may not be connected")
            except Exception as e:
                if debug_print and _dbg_loop_count % 100 == 0:
                    print(f"[WARN] HUD rendering failed: {e}")
        ...
        if debug_print:
            try:
                ...
                print(f"\nOscillation Metrics (Baseline):")
                # Brief analysis of the oscillation log
                try:
                    import pandas as pd
                    osc_df = pd.read_csv(os.path.join(log_dir, "oscillation_baseline.csv"))
                    
                    straight_data = osc_df[osc_df['mode_zone'] == 'STRAIGHT']
                    if len(straight_data) > 0:
                        print(f"  Straightaways:")
                        print(f"    - Average CTE variance: {straight_data['cte_variance_m2'].mean():.6f} m²")
                        print(f"    - Peak steering delta: {straight_data['steer_delta_rad'].max():.4f} rad/frame")
                        print(f"    - Average steering delta: {straight_data['steer_delta_rad'].mean():.4f} rad/frame")
                    
                    curve_data = osc_df[osc_df['mode_zone'] == 'CURVE']
                    if len(curve_data) > 0:
                        print(f"  Curves:")
                        print(f"    - Average CTE variance: {curve_data['cte_variance_m2'].mean():.6f} m²")
                        print(f"    - Peak steering delta: {curve_data['steer_delta_rad'].max():.4f} rad/frame")
                except Exception as e:
                    if debug_print:
                        print(f"  [Note] Could not calculate summary: {e}")
...
def main() -> None:
    ...
    ap.add_argument("--speed", type=float, default=0.4)  # PHASE 2: Reduced for oscillation damping
