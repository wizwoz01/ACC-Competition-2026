...
def detect_adjacent_line(img: np.ndarray, roi_y_frac: Tuple[float, float] = (0.6, 0.95),
                         min_length_px: int = 80, max_gap: int = 20) -> Tuple[bool, Tuple[int,int,int,int] | None]:
    """Detect a near-horizontal painted line in the lower part of the image.
    Returns (found, (x1,y1,x2,y2)) where coords are image pixel coordinates.
    """
    if img is None or img.size == 0:
        return False, None
    H, W = img.shape[:2]
    y0 = int(max(0, min(H - 1, roi_y_frac[0] * H)))
    y1 = int(max(0, min(H - 1, roi_y_frac[1] * H)))
    roi = img[y0:y1, :]
    if roi is None or roi.size == 0:
        return False, None
    gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
    # Enhance bright painted lines
    blurred = cv2.GaussianBlur(gray, (5,5), 0)
    _, th = cv2.threshold(blurred, 200, 255, cv2.THRESH_BINARY)
    edges = cv2.Canny(th, 50, 150)
    min_len = max(16, min_length_px)
    try:
        lines = cv2.HoughLinesP(edges, 1, np.pi/180.0, threshold=40, minLineLength=min_len, maxLineGap=max_gap)
    except Exception:
        lines = None
    if lines is None:
        return False, None
    for ln in lines:
        ...
        if abs(y2l - y1l) > 12:
            continue
        # accept line
        return True, (x1, y1_img, x2, y2_img)
    return False, None
...
def run_lane_only(actor: int, rate_hz: float, speed_mps: float, debug: bool,
    ...
    try:
        while True:
            ...
            try:
                ...
                if car_x is not None:
                    for i, ms in enumerate(ALL_MAP_SIGNS):
                        ...
                        # detect adjacent painted line (stop line) in camera only when needed
                        line_found, line_coords = detect_adjacent_line(bgr)

                        if ms.sign_type == 'stop':
                            # require both image label and a visible painted stop line
                            if 'stop' in lname and line_found:
                                sign_state['sign_last_trigger'][i] = now()
                                sign_state['stop_sign_active'] = True
                                sign_state['stop_sign_stopped_t'] = 0.0
                                sign_state['last_line'] = (line_coords, now())
                                lastp = sign_state.get('print_last', {}).get(i, 0.0)
                                if now() - lastp > 0.5:
                                    print(f"[SIGN] STOP triggered (map+image+line) dist={dist:.2f}")
            ...
            try:
                # capture previous active states for printing transitions
                prev_v = v_cmd
                applied = []
                with sign_lock:
                    tcur = now()
                    prev_stop = sign_state.get('stop_sign_active', False)
                    ...
                    if sign_state.get('stop_sign_active', False):
                        if sign_state.get('stop_sign_stopped_t', 0.0) <= 0.0:
                            ...
                            v_cmd = 0.0
                            applied.append('STOP_SIGN (start)')
                        else:
                    ...
                    # Print transitions: stop -> resume, yield -> resume, tl red -> resume
                    now_after = now()
                    cur_stop = sign_state.get('stop_sign_active', False)
                    cur_yield = (now_after < sign_state.get('yield_slow_until', 0.0))
                    cur_tl_red = (now_after < sign_state.get('tl_red_until', 0.0))
                    # stop resumed
                    if prev_stop and not cur_stop:
                        print(f"[SIGN] Resumed from STOP at t={now_after:.2f}")
                    # yield resumed
            ...
            try:
                ...
                if draw_detection:
                    ...
                    try:
                        ...
                        pass
                # draw detected stop line if available
                try:
                    if last_line is not None:
                        coords, lt = last_line
                        if coords is not None and (now() - lt) < 3.0:
                            x1, y1, x2, y2 = coords
