...
% and a simple pure-pursuit steering command.
%
% Output ports:
...
% Ensure project functions are on path
...
if exist(modelFile, 'file')
    try
        ...
        % If the file is locked/open, Simulink will prompt on overwrite.
    end
end
...
% Mission + steering S-Function block (avoids Stateflow/MATLAB Function)
sfunPath = [model '/TaxiStack'];
pos = [x0+dx y0+dy x0+dx+200 y0+dy+140];

% IMPORTANT: The generic "S-Function" block supports only Level-1 MATLAB
% S-functions. We need the Level-2 MATLAB S-Function block.
added = false;
libCandidates = { ...
...
% Set the referenced MATLAB function name (parameter name varies by version)
...
%
% The mask parameter name differs across Simulink releases. We inspect the
% dialog parameters and set the one that looks like it controls the MATLAB
% file/function name.
    ...
    try
        ...
        % Older versions may not expose DialogParameters; fall back to common names.
    end

    % Build candidate parameter list.
    % Prefer only parameters that actually exist on this block. If we can't
    % introspect, fall back to common names.
    candidates = {};
    names = {};
    try
        names = fieldnames(dp);
    ...
    if ~isempty(names)
        ...
        elseif any(strcmp(names, 'MATLABFile'))
            ...
            % Unknown naming; try a short list.
    ...
    % 1) Try direct set_param on the chosen dialog parameter
    ...
    % 2) Fallback: if direct set failed, try writing the mask value.
    if ~setOk
        try
            ...
            if isempty(idx)
                % Sometimes the label is different; try any mask name containing 'function'.
